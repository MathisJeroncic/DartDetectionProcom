...
class GetScores():
    ...
    def process_yolo_output(self, output):
        ...
        for i in range(len(classes)):
            ...
            print("trying to predict something from the stream")
            print(classes)
            print(dart_coords)

            # ✅ Conversion robuste : aplatissement + conversion GPU → CPU
            cleaned_coords = []
            for coord_pair in dart_coords:
                pair = []
                for c in coord_pair:
                    if torch.is_tensor(c):
                        pair.append(c.detach().cpu().item())
                    else:
                        pair.append(c)
                cleaned_coords.append(pair)

            dart_coords = np.array(cleaned_coords) if cleaned_coords else np.empty((0, 2))
            
            print("Returning calibration_coords:", calibration_coords)
            print("Returning dart_coords:", dart_coords)
...
"""
        ...
        # dart_coords = dart_coords.detach().cpu().numpy()
    ...
    def process_yolo_output(self, output):
        ...
        for i in range(len(classes)):
            ...
            # Extract xy norm coords safely (CPU)
    ...
    def find_homography(self, calibration_coords, image_shape):
        #print("calibration_coords homography : ",  calibration_coords)
